...
export default function VAFinancialSummaryView() {
  ...
  // Auto-expand the active file in the tree once it's known
  useEffect(() => {
  ...
  const handleFileSelect = (name: string) => {
    ...
    setPage(0);
    setExpandedFile(name); // expand its sheets in the tree
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] overflow-hidden bg-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              VA Results &mdash; {activeFile ?? "select a file"}
      ...
      {/* Body: left sidebar (files + sheets) | table area */}
              ...
              {files.map((f) => {
                ...
                return (
                  <div key={f.name}>
                    ...
                    <div
                      className={cn(
                        "group flex items-center gap-1 w-full px-2 py-1.5 border-l-2 cursor-pointer transition-colors",
                        isActiveFile
                    ...
                    >
                      <button
                        className="p-0.5 rounded text-slate-400 hover:text-slate-700 flex-shrink-0"
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleExpand(f.name);
                        }}
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className={cn(
                            ...
                            isActiveFile ? "text-primary" : "text-slate-700"
                          )}
                        >
                          {f.name}
                        </p>
                    ...
                    {/* Sheet children (only for the active+expanded file) */}
                    {isExpanded && isActiveFile && (
                      <div>
                        {sheets.length === 0 ? (
                          <p className="pl-9 pr-3 py-1 text-[11px] text-slate-400 italic">
                            No sheets
                          </p>
                        ) : (
                          sheets.map((s) => (
                            <button
                              ...
                              title={s}
                              className={cn(
                                "flex items-center gap-1.5 w-full text-left pl-9 pr-3 py-1 text-[11px] border-l-2 transition-colors",
                                s === activeSheet
